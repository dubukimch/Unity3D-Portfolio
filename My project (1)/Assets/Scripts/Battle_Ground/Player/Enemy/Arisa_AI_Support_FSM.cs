...
public class Arisa_AI_Support_FSM : MonoBehaviour
{
    ...
    public GameObject Enemy_Arisa_Player_UI;
    public Image Enemy_Arisa_Player_Hp;
    public Image Enemy_Arisa_Player_Mp;
    public Image Enemy_Arisa_Player_Exp;
    public Text Enemy_Arisa_Player_Level;

    float Enemy_Arisa_Hp_Value = 1f;
    float Enemy_Arisa_Mp_Value = 1f;
    float Enemy_Arisa_Exp_Value = 0f;
    ...
    void Update()
    {
        if (gameObject.GetComponent<Enemy_Arisa_AI_FSM>().enabled == true)
        {
            ...
            if (Attack_Judgment == true)
            {
                ...
                if (gameObject.tag == "Enemy_Top" || gameObject.tag == "Enemy_Middle" || gameObject.tag == "Enemy_Bottom")
                {
                    if (gameObject.tag == "Enemy_Top")
                        ForTop_Name = "Our_Right_ForeTop";
                    else if (gameObject.tag == "Enemy_Middle")
                        ForTop_Name = "Our_Middle_ForeTop";
                    else if (gameObject.tag == "Enemy_Bottom")
                        ForTop_Name = "Our_Left_ForeTop";
            ...
            else if (Attack_Judgment == false)
            {
                ...
                else if (Enemy_Arisa_Hp_Value >= 1f && Enemy_OurTeam_distance <= 9f) ;
                Attack_Judgment = true;
            }
        ...
        try
        {
            ...
            for (int i = 0; i < _target_OurTeam.Length; i++)
            {
                     ...
                     _targetTf_OurTeam.tag == "Enemy_Support" || _targetTf_OurTeam.tag == "Enemy_Bottom" || _targetTf_OurTeam.tag == "Enemy_Jungle"))
                {
                    ...
                    if (Enemy_OurTeam_distance <= 5f && Enemy_Arisa_Hp_Value > 0.3f)
                    {
                        Attack_Judgment = true;
                    }
                    else if (Enemy_OurTeam_distance > 5f || Enemy_Arisa_Hp_Value <= 0.3f || _target_OurTeam[i] == null || _target_OurTeam[i].tag == gameObject.tag)
                    {
    ...
    void Arisa_Status()
    {
        Enemy_Arisa_Player_Hp.fillAmount = Enemy_Arisa_Hp_Value;
        Enemy_Arisa_Player_Mp.fillAmount = Enemy_Arisa_Mp_Value;
        Enemy_Arisa_Player_Exp.fillAmount = Enemy_Player_UI_Manager.Instance.Enemy_Support_Player_Exp;
        Enemy_Arisa_Player_Level.text = "" + Enemy_Player_UI_Manager.Instance.Enemy_Support_Player_Level;

        if (Enemy_Arisa_Hp_Value >= 1f)
            Enemy_Arisa_Hp_Value = 1f;
        if (Enemy_Arisa_Mp_Value >= 1f)
            Enemy_Arisa_Mp_Value = 1f;
        if (Enemy_Arisa_Exp_Value >= 1f)
            Enemy_Arisa_Exp_Value = 1f;
    }
    void Enemy_ForeTop_Set_Value(string ForTop_Name)
    {
        ...
        try
        {
            ...
            {
                Enemy_ForeTop_target = GameObject.FindWithTag("Our_Last_ForeTop").transform;
                ForeTop_dir = Enemy_ForeTop_target.transform.position - transform.position;
                ForeTop_dir.y = 0;
                Enemy_ForeTop_Attack_distance = Vector3.Distance(Enemy_ForeTop_target.position, transform.position);
            }
    ...
    void TeamPlayer_Set_Value()
    {
        try
        {
            ...
            {
                Enemy_TeamPlayer_target_Value = false;
                Debug.Log("아군 이름" + Enemy_TeamPlayer_target);
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (gameObject.GetComponent<Enemy_Arisa_AI_FSM>().enabled == true)
        {
            if (other.gameObject.layer == LayerMask.NameToLayer("Monster_Attack"))
            {
                ...
                Enemy_Arisa_Hp_Value -= 15f * Time.deltaTime;
            ...
            if (other.gameObject.layer == LayerMask.NameToLayer("Own_Attack"))
            {
                ...
                Enemy_Arisa_Hp_Value -= 5f * Time.deltaTime;
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        {
            Enemy_Arisa_Hp_Value += 3f * Time.deltaTime;
