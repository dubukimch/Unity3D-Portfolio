...
public class Enemy_SopphiArt_AI_FSM : MonoBehaviour
{
    enum EnemyState
    {
        Stance,
    ...
    public GameObject Enemy_SopphiArt_Player_UI;
    public Image Enemy_SopphiArt_Player_Hp;
    public Image Enemy_SopphiArt_Player_Mp;
    public Image Enemy_SopphiArt_Player_Exp;
    public Text Enemy_SopphiArt_Player_Level;

    float Enemy_SopphiArt_Player_Hp_Value = 1f;
    float Enemy_SopphiArt_Player_Mp_Value = 1f;
    float Enemy_SopphiArt_Player_Exp_Value = 0f;
    ...
    void Update()
    {
        if (gameObject.GetComponent<Enemy_SopphiArt_AI_FSM>().enabled == true)
        {
            rg.velocity = Vector3.zero;
            Arisa_Status();

            Spin();
            Follow_go_Skill_Attack();
            SearchOurTeam();
            Look_OurTeam_Target();
            if (Enemy_SopphiArt_Player_Hp_Value <= 0)
                Die();
            ...
            else if (Attack_Judgment == false)
            {
                ...
                else if (Enemy_SopphiArt_Player_Hp_Value >= 1f && Enemy_OurTeam_distance <= 9f) ;
                Attack_Judgment = true;
            }
        ...
        try
        {
            ...
            for (int i = 0; i < _target_OurTeam.Length; i++)
            {
                     ...
                     _targetTf_OurTeam.tag == "Enemy_Support" || _targetTf_OurTeam.tag == "Enemy_Bottom" || _targetTf_OurTeam.tag == "Enemy_Jungle"))
                {
                    ...
                    if (Enemy_OurTeam_distance <= 5f && Enemy_SopphiArt_Player_Hp_Value > 0.3f)
                    {
                        Attack_Judgment = true;
                    }
                    else if (Enemy_OurTeam_distance > 5f || Enemy_SopphiArt_Player_Hp_Value <= 0.3f || _target_OurTeam[i] == null || _target_OurTeam[i].tag == gameObject.tag)
                    {
    ...
    void Arisa_Status()
    {
        Enemy_SopphiArt_Player_Hp.fillAmount = Enemy_SopphiArt_Player_Hp_Value;
        Enemy_SopphiArt_Player_Mp.fillAmount = Enemy_SopphiArt_Player_Mp_Value;
        Enemy_SopphiArt_Player_Exp.fillAmount = Enemy_Player_UI_Manager.Instance.Enemy_Jungle_Player_Exp;
        Enemy_SopphiArt_Player_Level.text = "" + Enemy_Player_UI_Manager.Instance.Enemy_Jungle_Player_Level;

        if (Enemy_SopphiArt_Player_Hp_Value >= 1f)
            Enemy_SopphiArt_Player_Hp_Value = 1f;
        if (Enemy_SopphiArt_Player_Mp_Value >= 1f)
            Enemy_SopphiArt_Player_Mp_Value = 1f;
        if (Enemy_SopphiArt_Player_Exp_Value >= 1f)
            Enemy_SopphiArt_Player_Exp_Value = 1f;
    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (gameObject.GetComponent<Enemy_SopphiArt_AI_FSM>().enabled == true)
        {
            if (other.gameObject.layer == LayerMask.NameToLayer("Monster_Attack"))
            {
                ...
                Enemy_SopphiArt_Player_Hp_Value -= 15f * Time.deltaTime;
            ...
            if (other.gameObject.layer == LayerMask.NameToLayer("Own_Attack"))
            {
                ...
                Enemy_SopphiArt_Player_Hp_Value -= 5f * Time.deltaTime;
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        {
            Enemy_SopphiArt_Player_Hp_Value += 3f * Time.deltaTime;
